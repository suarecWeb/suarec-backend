...
export class WorkContractService {
  ...
  async create(createWorkContractDto: CreateWorkContractDto): Promise<WorkContract> {
    try {
      const { clientId, providerId, publicationId, ...contractData } = createWorkContractDto;
      ...
      if (!provider) {
        throw new BadRequestException(`Provider with ID ${providerId} not found`);
      }

      // Verificar que no sean la misma persona
      if (clientId === providerId) {
        throw new BadRequestException('Client and provider cannot be the same person');
      }

      let publication = null;
      if (publicationId) {
        publication = await this.publicationRepository.findOne({ where: { id: publicationId } });
        if (!publication) {
  ...
  async findAll(paginationDto: PaginationDto): Promise<PaginationResponse<WorkContract>> {
    try {
  ...
  async findByUser(userId: number, paginationDto: PaginationDto, role?: 'client' | 'provider'): Promise<PaginationResponse<WorkContract>> {
    try {
      const { page = 1, limit = 10 } = paginationDto;
      const skip = (page - 1) * limit;
  ...
  async getUserWorkHistory(userId: number): Promise<{
    ...
    try {
      ...
      const clientStats = {
        ...
        completed: clientContracts.filter(c => c.status === ContractStatus.COMPLETED).length,
        inProgress: clientContracts.filter(c => c.status === ContractStatus.IN_PROGRESS).length,
        totalSpent: clientContracts
          .filter(c => c.status === ContractStatus.COMPLETED && c.agreed_price)
      ...
      const providerStats = {
        ...
        completed: providerContracts.filter(c => c.status === ContractStatus.COMPLETED).length,
        inProgress: providerContracts.filter(c => c.status === ContractStatus.IN_PROGRESS).length,
  ...
  async findOne(id: string): Promise<WorkContract> {
    try {
      const workContract = await this.workContractRepository.findOne({
        where: { id },
        relations: ['client', 'provider', 'publication', 'ratings', 'ratings.reviewer'],
      });
  ...
  async update(id: string, updateWorkContractDto: UpdateWorkContractDto): Promise<WorkContract> {
    try {
      ...
      if (updateWorkContractDto.status) {
        ...
        const validTransitions = {
          ...
          [ContractStatus.ACCEPTED]: [ContractStatus.IN_PROGRESS, ContractStatus.CANCELLED],
          [ContractStatus.IN_PROGRESS]: [ContractStatus.COMPLETED, ContractStatus.CANCELLED, ContractStatus.DISPUTED],
        ...
        if (newStatus === ContractStatus.COMPLETED && !updateWorkContractDto.end_date) {
  ...
  async remove(id: string): Promise<void> {
    try {
      ...
      if (![ContractStatus.PENDING, ContractStatus.CANCELLED].includes(workContract.status)) {
        throw new BadRequestException('Only pending or cancelled contracts can be deleted');
      }
