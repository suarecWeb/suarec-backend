...
import { Contract, ContractBid, ContractStatus } from './entities/contract.entity';
...
export class ContractService {
  ...
  async createBid(createBidDto: CreateBidDto): Promise<ContractBid> {
    ...
    if (contract.status !== ContractStatus.NEGOTIATING && contract.status !== ContractStatus.PENDING) {
      throw new BadRequestException('El contrato no está disponible para ofertas');
    }
    ...
    const recipient = contract.client.id === bidderId ? contract.provider : contract.client;
  ...
  async acceptBid(acceptBidDto: AcceptBidDto): Promise<Contract> {
    ...
    if (bid.contract.client.id !== acceptorId && bid.contract.provider.id !== acceptorId) {
      throw new BadRequestException('No tienes permisos para aceptar esta oferta');
    }
    ...
    const recipient = bid.contract.client.id === acceptorId ? bid.contract.provider : bid.contract.client;
  ...
  async getContractsByUser(userId: number): Promise<{ asClient: Contract[], asProvider: Contract[] }> {
  ...
  async cancelContract(contractId: string, userId: number): Promise<Contract> {
    ...
    if (contract.client.id !== userId && contract.provider.id !== userId) {
      throw new BadRequestException('No tienes permisos para cancelar este contrato');
    }
  ...
  async getPublicationBids(publicationId: string): Promise<{ contracts: Contract[], totalBids: number }> {
    ...
    const totalBids = contracts.reduce((total, contract) => total + contract.bids.length, 0);
