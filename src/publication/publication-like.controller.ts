...
export class PublicationLikeController {
  ...
  async unlikePublication(
    ...
    await this.publicationLikeService.unlikePublication(req.user.id, publicationId);
  ...
  async getPublicationLikesCount(@Param('id') publicationId: string) {
    const count = await this.publicationLikeService.getPublicationLikesCount(publicationId);
    return { count };
  }
  ...
  async checkUserLike(
    ...
    const hasLiked = await this.publicationLikeService.hasUserLiked(req.user.id, publicationId);
    return { hasLiked };
  }
